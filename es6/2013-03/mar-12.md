...
BM: Background as trained language designers and here to help.

JP: Also excited about asm.js

BM: Not sure the spec status.
...
MM: The way that you properly reject our favorite fixes, I thin you should apply to your favorite fixes
...
b/c generators can return an argument, if you're using a
...
AWB: I noticed this in the examples shown in the

WH: Why I'm in favor… Throwing StopIteration across places where no iterators exist and if an iterator is present, you'll experience unexpected behavior.
...
MM: My proposal is that you provide it (generator object) as a parameter of next(), every time.

DH: (clarifying) "iteration" is one time around the loop. "loop" is the entire the operation.

WH: It's possible for next() to cause havoc from one iteration to another by
...
EA: You can have a return value in a generator
...
ARB: There is another secondary effect on GC
...
ARB: This is actually the path you'd want in typed languages
...
**Conclusion/Resolution**
- StopIteration is dead.
- Agreement on pure, stateless version of:
```js
{
  next() -> { done: false , value: any }
          | { done: true[, value: any] }
...
WH: Why would you want to do this?
...
ARB: Every single construct, import, loading etc now has two different mechanisms to support.
...
BM: Thought that the dynamic checks were too late
...
STH: (refers to an early slide)

MM: (confirms that sam answers Q)
...
ARB: When you evaluate a script that has module Foo, veal of that script registers something in the module registry and then evaluate a script that has a module foo, you are involved in a shared module state
...
ARB: We're over-prioritizing for concatenation. I think concatenation is vastly over-rated. The language shouldn't be hostile, but should stop at good enough. We shouldn't optimize the design of the language around a secondary concept
...
ARB: I think that mid-term concatenation will harm browsers

YK: Do you think that concatenation will go away?

ARB: In the long term, yes
...
ARB: How about adding a way to plug in a string as the body of a module as if it was a file.
...
ARB: It is a string so the string only needs to be lexed, then the parsing etc can be paralellized
...

ARB: There is no way to not put the names in the global registry.

DH: The file system (in SML) is also a shared registry. The module registry is no different

ARB: There is no way to create a local module
...
ARB: Would prefer for the default behavior to error, need to be explicit if you want module to override.
...
STH: This is something to be deferred without blocking progress


**Conclusion/Resolution**
Default: declarative form is an error if it already exists in the module registry.

Override with `System.set`
