...
SYG: Original bug was found in Web Assembly. Wasm is using MM for interop. Conrad who is the author found is on the call. The wasm paper has fixed it in a similar way so he will talk.
...
Conrad SYG: Compilers aren’t going to do something like this. The wasm fix, allows 2,1 to be printed here. The wasm fix isn’t as far as I know allowing compiler optimizations but.
...
Conrad: The strong fix there could be compiler optimizations but the weak fix does not.
...
Conrad: The strong fix is stronger than that -- it introduces a not complete a notion of total ordering. It does change the semantics… abstracting the str
...
Conrad: My intuition is to allow the minimalistic fix… you are opening up to having weird architecture for optimizations.
...
Conrad: The thing about this is that these two examples are disallowed by 2 separate rules.
...
SYG: The paper is not published but recommend people to read it once it is. Conrad are you allowed to share the draft? With permission, I’ll post to reflector.

Conrad: No. Especially if you are a reviewer of PLDI.

SYG: People who want to email me, can read it unless you are a reviewer.

WH: In addition to the strong and weak solutions, there was an intermediate solution on GitHub, correct? What are Conrad’s thoughts about it?

Conrad: Thoughts on intermediate… I strongly disapprove of the strong fix because of the problems it causes. Intermediate fix is unclear; its one advantage is that it the makes the memory model smaller, but that’s not necessarily something we should be optimizing. I want to give a little proof that the intermediate the solution doesn’t have issues.

WH: Are we convinced the weak solution will fix the problems?

Conrad: Yes. There is a proof in the paper.

SYG: We don’t understand the full ramifications of the intermediate solution.

WH: [Responding to Conrad] Good. Then I agree we should go with the weak solution.
...
Brad: I didn’t actually… get reviewers… long time ago we talked about this and we wanted to push to stage3 … Only people who looked at it was the last time at the meeting. WH gave a review and thumbs up?
...
RonB: Mentioned in issue thread I have concerns that using the initializer in initializer description somewhat conflates it. It feels like we should have a different if you’re not initializing a value.

DE: I misunderstood your review on the issue thread. What should the name be?

RonB: We discussed finishers?
...
WH: My comment is very similar to RonB’s. I too was confused by overloading the name “initializer”, wondering how initializing a nonexistent property works.
...
RonB: I had issue with the breakout session discussion because people on the call weren’t able to participate. Don’t feel like there’s full consensus yet.

DE: Where do you want to go from here?

RonB: In most discussions and the community and comment threads, it’s a 50/50 split on the in favor of decorators before export? Doesn’t seem locked in.

DE: How long do we need?

RonB: One more meeting? 

DE: Let’s agree to try and conclude that in the January meeting.

RonB: I am in favor of doing it right vs pushing it out.
...
EF: As one of the resident grease monkeys about this I want to hear more from implementers about the complexity and optimizability of this…. Nothing?
...
EF: My intuition is that when you codify this a first class language thing
...
RB: My concern is the slot is forgeable with or without eval. If I create a template literal and forge it then it will be a template literal.
...
Natalie: Is this ever going to be used? This enables people to write secure code but it’s even better if people don’t have to think about this. Is this primarily for people who are using for security? Anyway we can enforce URL rather than opt in?

DE: Do the headers help?

Natalie: only if people use them.
...
Bradley: I think we are skipping over how these template object arrays are a little string? The host could technically do things like source code manipulation… Do people really feel strongly of having these really strange object arrays and somehow preserving where they are in sourcetext.

MM: Yes.

Bradley: How is an internal field different from callsite potency?

MM: I don’t understand.

Bradley: You can store strange internal state through side channels, so you have persistent state on this special object which can’t be reproduced in a proxy..
...
[proposal](https://github.com/tc39/proposal-ecma402-datetime-style)
...
```
new Intl.DateTimeFormat(“en”m { timeStyle: “”, dateStyle: “” });
```
...
FT: if there already a way for OS to give preference, it can set it. If OS has a setting then that is an issue.
...
FT: What does the UA provided mean?

MF: If a user has preferences for date/time style and those preferences are quite specific...

FT: I think we can definitely work on a different proposal to allow more formats to intl date format. This doesn’t prohibit that.

MF: Satisfies me.

#### Conclusion
No objections to stage 2
Stage 3 reviewers: FT, NHD

## Hashbang Grammer Revisit
No objections to stage 3
...
MA: Bradley is pointing out that you don’t have access to private fields 
...
RB: I want to make the point for triple bar, when we look at double bar where we are doing boolean OR—would triple bar only care about undefined or would it also care about boolean values? It feels strange/doesn’t feel like it’s the right semantic for that syntax.
