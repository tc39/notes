...
ACE:  Yeah, so something else about our use at Bloomberg. So we have an implementation of this not, using the syntax but it is available in ESM, and the real amazing thing for us is
...
The main points of the discussion were: How does this interact with Wasm modules? The current Wasm-ESM integration marks them as async. However, it may be possible to make their evaluation/instantiation synchronous and thus allow them to be deferred. So far the performance recommendation has been to "make things async" using dynamic import(). With this proposal, using `await` at the top-level of a module would prevent it from being deferred, going against that recommendation. How can we reconcile this? Forcing the namespace imports syntax may make it less statically analyzable for tools. However, it's likely that current heuristics (e.g., “it works as long as all accesses are . and not []”) are already good enough for this type of static analysis. Forcing the namespace syntax is also a compromise for ergonomics/orthogonality.
